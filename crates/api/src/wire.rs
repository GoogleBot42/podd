...
/// The settings DTOs live in `podd-core` (the daily-reboot scheduler and the
/// schedule overrides consume them); the wire shape is unchanged — they are
/// free-sleep's `settings.json` types.
pub use podd_core::settings::{
    AlarmBehavior, AlarmOverride, InactiveAlarmBehavior, PrimePodDaily, ScheduleOverrides,
    Settings, SideSettings, TapConfig, TapsConfig, TempChange, TemperatureFormat,
    TemperatureScheduleOverride,
};
